...
\begin{enumerate}
  ...
  <<>>=
    cars_dt[, !"car_name", with=FALSE][,
                                       lapply(.SD,FUN=function(col){
                                         print(names(col))
    gg_plot <- ggplot(data=data.frame(X=col), aes(x=X)) + geom_histogram(aes(y=..density..))
    if (!is.factor(col)) {
      gg_plot <- gg_plot + stat_density(fill=NA,color='red')
    }
    show(gg_plot)
  })]
  @
